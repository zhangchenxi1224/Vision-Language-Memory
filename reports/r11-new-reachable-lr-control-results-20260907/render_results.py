"""Verify, extract selected evidence, and render the reachable low-LR delivery."""
...
ARCHIVE = ROOT / "raw" / "r11-new-lr-af00d04-20260907-round02.tar.gz"
ARCHIVE_SHA256 = "9bfe553c8aa7a8eef8a69dcc19ef9d37fa182c7766548dc81b5df39737acfcd1"
FAILURE_ARCHIVE = ROOT / "raw" / "r11-new-lr-88a0b48-20260907-round01-technical-failure.tar.gz"
FAILURE_ARCHIVE_SHA256 = "89d893798d96fe9d338f83cc5fdc75cc2fb1aff9ec11b18b1ec3529e889f770f"
FAILURE = "r11-new-lr-88a0b48-20260907-round01-preflight"
PREFLIGHT = "r11-new-lr-af00d04-20260907-round02-preflight"
FORMAL = "r11-new-lr-af00d04-20260907-round02-formal"
CONDITIONS = ("lr-005", "lr-001")
STEPS = (0, 64, 128, 192, 256)
COLORS = {"lr-005": "#1D4ED8", "lr-001": "#D97706"}
...
def materialize_and_audit(bundle: tarfile.TarFile, config: dict) -> dict:
    with tempfile.TemporaryDirectory(prefix="vlm-r11-low-lr-audit-") as temporary:
...
def save_failure_evidence() -> dict:
    if sha256_file(FAILURE_ARCHIVE) != FAILURE_ARCHIVE_SHA256:
        raise ValueError("Technical-failure archive SHA-256 mismatch.")
    destination_root = ROOT / "technical-failure-round01"
    selected = ("artifact_inventory.json", "config.json", "launch.json", "stderr.log", "stdout.log", "terminal.json")
    with tarfile.open(FAILURE_ARCHIVE, "r:gz") as bundle:
        validate_members(bundle, (FAILURE,))
        inventory = verify_inventory(bundle, FAILURE)
        for relative in selected:
            destination = destination_root / relative
            destination.parent.mkdir(parents=True, exist_ok=True)
            destination.write_bytes(read_member(bundle, f"{FAILURE}/{relative}"))
        terminal = read_json(bundle, f"{FAILURE}/terminal.json")
    if terminal.get("status") != "technical_failed" or "diffusers" not in terminal.get("error", ""):
        raise ValueError("Unexpected round01 failure record.")
    return {"inventory": inventory, "terminal": terminal}
...
def main() -> None:
    ...
    failure_record = save_failure_evidence()

    teacher_x_t = target["teacher_x_T_fp32"]
    source_x_t = target["source_only_x_T_init_fp32"]
    condition_geometry = {}
    for condition in CONDITIONS:
        alpha = float(result["conditions"][condition]["initialization_alpha"])
        base_learning_rate = float(result["conditions"][condition]["base_learning_rate"])
    ...
    axes[0, 0].set_title("Lower LR removes overshoot but reaches a floor")
    ...
    axes[0, 1].set_title("Neither LR achieves the required 99% reduction")
    ...
    axes[1, 0].set_title("First-step update is now smaller than remaining distance")
    axes[1, 0].set_ylabel("xT-space L2 norm (log)")
    axes[1, 0].legend(fontsize=8)

    axes[1, 1].set_yscale("log")
    axes[1, 1].set_title("Both LRs settle away from the exact xT")
    axes[1, 1].set_xlabel("Optimizer step")
    axes[1, 1].set_ylabel("xT L2 distance to teacher (log)")
    axes[1, 1].legend(fontsize=8)
    for axis in axes.flat:
        axis.grid(alpha=0.2)
    fig.savefig(figures / "low_lr_diagnostics.png", dpi=180,
                metadata={"Software": "Vision-Language-Memory"})
    plt.close(fig)

    montage = [
        (ROOT / "images/fixed-target.png", "Fixed reachable target"),
        (ROOT / "images/lr-005-step-000.png", "LR=.005 / step 0"),
        (ROOT / "images/lr-005-step-256.png", "LR=.005 / step 256"),
        (ROOT / "images/lr-001-step-000.png", "LR=.001 / step 0"),
        (ROOT / "images/lr-001-step-256.png", "LR=.001 / step 256"),
    ]
    ...
    fig.suptitle("Fixed-target low-LR control remains non-semantic", fontsize=13)
    fig.savefig(figures / "low_lr_endpoint_montage.png", dpi=150,
                metadata={"Software": "Vision-Language-Memory"})
    plt.close(fig)

    summary = {
        "schema": "vision_memory.r11-new-reachable-lr-control-derived-summary.v1",
        "archive": {"path": ARCHIVE.relative_to(ROOT).as_posix(), "sha256": ARCHIVE_SHA256},
        "technical_failure_archive": {
            "path": FAILURE_ARCHIVE.relative_to(ROOT).as_posix(),
    ...
    manifest = {
        "schema": "vision_memory.r11-new-reachable-lr-control-delivery-manifest.v1",
