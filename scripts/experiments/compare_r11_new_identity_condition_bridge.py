"""Independent raw-artifact aggregation for the R11_new identity-condition bridge."""
...
def _validate_parent_bridge(config: Mapping[str, Any]) -> dict[str, Any]:
    ...
    parent_config = CONFIG.with_name("r11_new_canonical_latent_bridge_target01_source_only_init.json")
    ...
    if parent.get("protocol") != config["parent_bridge"]["protocol"]:
        raise ValueError("Identity bridge comparator is not the locked source-only/original-event arm.")
    expected = {
...
def _validate_manifest(
    ...
    *,
    ...
    if (
        ...
        or changed != condition_binding.single_changed_factor(config)
        or not core.validate_bridge_information_boundary(information or {})
    ...
    if not condition_binding.validate_manifest_identity(manifest, config=config):
        raise ValueError("Identity bridge raw condition/probe/original-event binding drifted.")
...
def _write_report(path: Path, comparison: Mapping[str, Any]) -> None:
    ...
    lines = [
        ...
        f"- 次级条件诊断：`{comparison['secondary_solver_hypothesis_decision']}`",
        "",
        "## 证据边界",
        "",
...
def compare(
    *,
    ...
    if (
        ...
        or summary.get("secondary_solver_hypothesis_audit") != conditioning_audit
        or summary.get("secondary_solver_hypothesis_decision") != conditioning_decision
        or summary.get("formal_success_gate") is not False
