"""Independently audit PRP+ output and its exact first-step parent reproduction."""
...
def _verify_parent_bindings(parent_root: Path, contract: Mapping[str, Any]) -> dict[str, Any]:
    ...
    for name, (path, expected) in bindings.items():
        core.require(path.is_file() and core.sha256_file(path) == expected, f"PRP+ parent {name} drift.")
    ...
    core.require(
        ...
        "PRP+ parent scientific identity drift.",
    )
    return {
        "passed": True,
        "audit": parent_audit,
...
def audit_parent_first_step(output_root: Path, parent_root: Path | None = None) -> dict[str, Any]:
    ...
    core.require(run_audit["mode"] == "formal", "Parent audit requires formal PRP+ output.")
    ...
    core.require(iteration_exact and candidates_exact, "PRP+ first step does not exactly reproduce the parent.")
    core.require(
        ...
        "PRP+ initial recurrence metadata drift.",
    )
    return {
        "passed": True,
