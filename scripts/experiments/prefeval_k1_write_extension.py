...
from scripts.experiments.prefeval_k1_refresh_bank import save_once, archive_uncommitted_tail
...
def train(args, pipe, rows):
    ...
    if args.continue_from:
        manifest['continued_from_sha256'] = sha(args.continue_from)
    ...
    if checkpoint.exists():
        ...
        previous = json.loads((args.continue_from.parent/'manifest.json').read_text())
        validate_extension(previous, manifest, step=23360, cursor=93440)
        saved = load_training_checkpoint(args.continue_from, trainable_module=pipe.unet,
                                         optimizer=optimizer, expected_manifest=previous)
        first = saved['optimizer_step']
        validate_extension(previous, manifest, step=first, cursor=saved['episode_cursor'])
        if not (args.output/'optimization.jsonl').exists():
            shutil.copyfile(args.continue_from.parent/'optimization.jsonl', args.output/'optimization.jsonl')
        save_once(args.output/'continuation.json', {'source':str(args.continue_from),
            'source_sha256':manifest['continued_from_sha256'], 'restored_step':first,
            'restored_optimizer_states':len(saved['optimizer']['state']),
            'restored_rng':True, 'next_draw':saved['episode_cursor'], 'final_step':args.steps})
    if args.continue_from:
        archive_uncommitted_tail(args.output/'optimization.jsonl', first)
        print(json.dumps({'resumed_at':first,'next_step':first+1,'final_step':args.steps,
                          'optimizer_states':len(optimizer.state)}),flush=True)
