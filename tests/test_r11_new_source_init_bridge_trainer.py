...
def test_preflight_reuses_the_single_registered_forward(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setattr(trainer, "_verify_source_only_initialization", lambda *_args, **_kwargs: True)
...
def test_formal_blocks_before_first_update_when_initialization_is_invalid(
    ...
    (output_dir / "manifest.json").write_text("{}\n", encoding="utf-8")
    initial_output = oracle()
    monkeypatch.setattr(trainer, "_verify_source_only_initialization", lambda *_args, **_kwargs: False)
    monkeypatch.setattr(
        trainer,
        "decode_model_latents_unit_interval",
        lambda *_args, **_kwargs: torch.zeros(1, 1, 1, 1),
...
def _initialization_case(tmp_path: Path, monkeypatch: pytest.MonkeyPatch, dtype: torch.dtype):
    ...
    initial_x_t, record = trainer._source_only_initialization(
        source_latents=source, scheduler=oracle.sampler, compute_device=source.device, output_dir=tmp_path,
    )
    with torch.no_grad():
...
def test_source_only_initializer_clones_source_and_uses_actual_native_reconstruction(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch, dtype: torch.dtype,
) -> None:
    oracle, _teacher, record, output, prepare_calls = _initialization_case(tmp_path, monkeypatch, dtype)
    expected_x_t = oracle.source_latents.float().clone()
    assert len(prepare_calls) == 1
    assert prepare_calls[0][1] == {"sigmas_are_effective": True}
    assert record["actual_effective_sigma0"] == output.effective_sigmas[0] != 0.5
    ...
    assert record["parameter_dtype"] == "torch.float32"
    assert record["compute_dtype"] == str(dtype)
    assert record["initial_x_t_equals_source"] is True
    assert record["initialization_teacher_assisted"] is False
    ...
    assert set(payload["tensor_sha256"]) == set(trainer.INITIALIZATION_TENSOR_KEYS)
    assert "teacher_fp32" not in payload
    assert "trajectory_point0_teacher_normalized_rmse" not in payload
    assert trainer._verify_source_only_initialization(record, oracle=oracle, initial_output=output)
    with torch.no_grad():
        oracle.x_T_fp32.add_(0.125)
    assert not trainer._verify_source_only_initialization(record, oracle=oracle, initial_output=output)
    with pytest.raises(ValueError, match="fresh artifact"):
        trainer._source_only_initialization(
            source_latents=oracle.source_latents, scheduler=oracle.sampler,
            compute_device=oracle.source_latents.device, output_dir=tmp_path,
        )


@pytest.mark.parametrize("dtype", [torch.float32, torch.bfloat16])
def test_initializer_has_no_teacher_query_channel_and_teacher_changes_do_not_change_x_t(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch, dtype: torch.dtype,
) -> None:
    oracle, teacher, record, _output, _calls = _initialization_case(tmp_path / "first", monkeypatch, dtype)
    assert set(inspect.signature(trainer._source_only_initialization).parameters) == {
        "source_latents", "scheduler", "compute_device", "output_dir",
    }
    replacement_teacher = teacher.add(99)
    monkeypatch.setattr(trainer.core, "BRIDGE_TEACHER_TENSOR_SHA256", trainer.canonical_tensor_sha256(replacement_teacher))
    replacement_x_t, replacement_record = trainer._source_only_initialization(
        source_latents=oracle.source_latents, scheduler=oracle.sampler,
        compute_device=oracle.source_latents.device, output_dir=tmp_path / "second",
    )
    assert torch.equal(replacement_x_t, oracle.initial_x_T_fp32)
    assert replacement_record["tensor_sha256"] == record["tensor_sha256"]
    assert replacement_x_t.data_ptr() != oracle.source_latents.data_ptr()
    for forbidden in ("teacher", "query", "choices", "answer", "target_index", "sample_id"):
        with pytest.raises(TypeError, match="unexpected keyword"):
            trainer._source_only_initialization(
                source_latents=oracle.source_latents, scheduler=oracle.sampler,
                compute_device=oracle.source_latents.device, output_dir=tmp_path / "forbidden",
                **{forbidden: replacement_teacher},
            )


@pytest.mark.parametrize("dtype", [torch.float32, torch.bfloat16])
@pytest.mark.parametrize("mutation", [
    "record_sigma", "output_schedule", "both_schedules", "both_nominal", "teacher_payload",
    "source_equality_flag", "teacher_assisted_flag", "parameter_dtype", "compute_dtype", "both_formula",
    "both_operator", "wrong_x_t", "wrong_start", "wrong_output_h0",
    "payload_device", "record_device", "both_device",
])
def test_source_only_initialization_rejects_resigned_mutations(
    ...
    elif mutation == "teacher_payload":
        payload["teacher_fp32"] = torch.zeros_like(payload["source_latents_fp32"])
    elif mutation == "source_equality_flag":
        payload["initial_x_t_equals_source"] = record["initial_x_t_equals_source"] = False
    elif mutation == "teacher_assisted_flag":
        payload["initialization_teacher_assisted"] = record["initialization_teacher_assisted"] = True
    elif mutation in {"parameter_dtype", "compute_dtype"}:
        payload[mutation] = record[mutation] = "forged dtype"
...
def test_run_installs_artifact_before_one_preflight_forward_and_uses_four_step_m0(
    ...
    def manifest(**kwargs):
        assert trainer._verify_source_only_initialization(
    ...
    assert saved_manifest["initialization_binding"]["initial_x_t_equals_source"] is True
    assert saved_manifest["initialization_binding"]["initialization_teacher_assisted"] is False
    checkpoint = torch.load(output_dir / "checkpoints" / "step-000.pt", weights_only=True)
    assert torch.equal(checkpoint["x_T_fp32"], source.float())
    assert not torch.equal(checkpoint["trajectory_fp32"][0], checkpoint["z_t_fp32"])
...
def test_source_only_entry_uses_new_core_config_and_protocol() -> None:
    assert trainer.core.__name__ == "vision_memory.training.r11_new_source_init_bridge"
    assert trainer.PROTOCOL == "R11-New-Canonical-Latent-Bridge-Source-Only-Init-Target01"
    assert trainer.CONFIG_PATH.name == "r11_new_canonical_latent_bridge_target01_source_only_init.json"
    assert trainer._load_config()["protocol"] == trainer.PROTOCOL
    assert not hasattr(trainer.core, "BRIDGE_START_TEACHER_NRMSE_MAX")
    assert trainer.INITIALIZATION_SCHEMA == "vision_memory.r11-new-source-only-initialization.v1"


def test_manifest_distinguishes_independent_initializer_from_teacher_supervised_optimization(monkeypatch) -> None:
    oracle = _ToyOracle()
    observed = {key: "hash" for key in (
        "blank_source_rgb_sha256", "source_latents_fp32_sha256", "initial_x_T_fp32_sha256",
    )}
    monkeypatch.setattr(trainer, "_parity_checks", lambda **_kwargs: {"passed": True, "observed": observed})
    monkeypatch.setattr(trainer.phase1a, "_writer_information_boundary", lambda _target: {"passed": True})
    monkeypatch.setattr(trainer.r5, "sha256_file", lambda _path: "datahash")
    monkeypatch.setattr(trainer.r8, "git_value", lambda *args: "" if args[0] == "status" else "a" * 40)
    target = SimpleNamespace(segment_id=trainer.core.BRIDGE_TARGET_SEGMENT_ID, to_dict=lambda: {"fixture": True})
    context = {"target": target, "selected": [target], "selected_sha": "selected",
               "source_rgb": torch.zeros(1), "source_latents": oracle.source_latents,
               "condition_record": {"sha256": "condition"}}
    manifest = trainer._manifest(
        args=SimpleNamespace(mode="technical-preflight", train=Path("train"), dev=Path("dev"),
                             dreamlite_device="cuda:0", reader_device="cuda:1"),
        context=context, oracle=oracle, initial_output=oracle(), parent_binding={},
        teacher_record={}, snapshot_bindings={}, determinism={}, initialization_binding={},
    )
    boundary = manifest["information_boundary"]
    assert trainer.core.validate_bridge_information_boundary(boundary)
    assert boundary["canonical_teacher_used_only_by_dense_loss"] is True
    assert boundary["canonical_teacher_used_by_initialization_and_dense_loss"] is False
    assert boundary["teacher_assisted_initialization"] is False
    assert manifest["single_changed_factor"] == {
    ...
    assert manifest["formal_success_gate"] is False and manifest["phase2_allowed"] is False
    assert manifest["fixed_contract"]["m0_definition"] == trainer._load_config()["unchanged_contract"]["m0_definition"]
