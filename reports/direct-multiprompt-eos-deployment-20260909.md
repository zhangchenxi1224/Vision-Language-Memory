...
00:44 已确认：新Direct完成48/96，随后两组U-Net也已绑定到同一个dl-base实例。trust上为新数据创建的两个等待进程已退出。原数据两组U-Net此前已各完成512步和评测，无需等待新Direct。详见 [同实例U-Net接续记录](direct-multiprompt-unet-dl-base-20260909.md)。
