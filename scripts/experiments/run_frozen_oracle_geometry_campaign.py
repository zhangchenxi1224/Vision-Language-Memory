...
def run_campaign(args):
    ...
    def _command_run(run, pair, mode="optimize"):
        ...
        cmd = [sys.executable, str(ROOT / "scripts/experiments/run_frozen_oracle_geometry_worker.py"),
    ...
    try:
        ...
        analyze = [sys.executable, str(ROOT / "scripts/experiments/analyze_frozen_oracle_geometry.py"),
                   "--root", str(args.output_root), "--manifest", str(args.manifest)]
        subprocess.run([*analyze, "--prepare-evaluations"], cwd=ROOT, check=True)
        evaluation_plan = load(args.output_root / "evaluation_inputs/plan.json")
        if evaluation_plan["status"] == "ready":
            atomic_json(args.output_root / "status.json", {"stage": "A5_A8", "state": "running", **identity})
            with concurrent.futures.ThreadPoolExecutor(max_workers=2) as pool:
                evaluations = pool.submit(command_run, evaluation_plan["evaluation_run"], (0, 1), "evaluate")
                def reopt_lane():
                    return [command_run(r, (2, 3)) for r in evaluation_plan["reoptimization_runs"]]
                reoptimization = pool.submit(reopt_lane)
                evaluations.result()
                reoptimization.result()
            subprocess.run(analyze, cwd=ROOT, check=True)
        decision = load(args.output_root / "analysis/geometry_decision.json")
        if decision.get("status") != "complete":
            raise RuntimeError("analysis did not verify complete geometry evidence")
        render_python = "/inspire/ssd/project/exploration-topic/czxs26210936/envs/vlm-r5-reporting-372fcbb/bin/python"
        render = subprocess.run([render_python if Path(render_python).exists() else sys.executable,
                                 str(ROOT / "scripts/experiments/render_frozen_oracle_geometry.py"),
                                 "--root", str(args.output_root)], cwd=ROOT)
        atomic_json(args.output_root / "render_terminal.json", {"returncode": render.returncode})
        atomic_json(args.output_root / "status.json", {
            "stage": "geometry_complete", "state": "completed",
            "figures_complete": render.returncode == 0,
            "next": "scientific review of canonicalization before independent bank/controller training", **identity})
