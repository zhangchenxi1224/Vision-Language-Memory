...
RUNNER = ROOT / "scripts/experiments/run_r11_new_reachable_basin_control.py"
...
def _metric(config: dict, condition: str, step: int) -> dict:
    return {
        ...
        "condition": condition,
        "alpha": 0.5 if condition == "alpha-050" else 0.99,
        "optimizer_step": step,
...
def test_config_is_immutable_fixed_target_and_diagnostic_only() -> None:
    ...
    assert config["status"] == "preregistered_after_reachable_alpha0_failure_before_any_warm_start_forward"
    assert config["fixed_target_contract"]["regenerate_noise_or_target_forbidden"] is True
    assert config["unchanged_optimizer_contract"]["condition_order"] == ["alpha-050", "alpha-099"]
...
def test_parent_hash_bindings_match_the_delivered_run() -> None:
    ...
    delivery = ROOT / "reports/r11-new-reachable-endpoint-control-results-20260907"
    ...
    archive = delivery / "raw/r11-new-reachable-986a181-20260907-round01.tar.gz"
    assert core.sha256_file(archive) == parent["source_archive_sha256"]
    member = (
        "r11-new-reachable-986a181-20260907-round01-formal/target/reachable_target.pt"
    )
...
def test_warm_starts_are_exactly_the_two_preregistered_interpolants() -> None:
    config = core.load_config()
    source = torch.zeros((1, 4, 128, 128), dtype=torch.float32)
    teacher = torch.ones_like(source)
    starts = {
        name: core.warm_start(source, teacher, float(item["alpha"]))
        for name, item in core.condition_map(config).items()
    }
    assert torch.equal(starts["alpha-050"], torch.full_like(source, 0.5))
    assert torch.equal(starts["alpha-099"], torch.full_like(source, 0.99))
    with pytest.raises(ValueError, match="alpha"):
        core.warm_start(source, teacher, 0.0)


def test_learning_rate_is_the_unchanged_parent_schedule() -> None:
    config = core.load_config()
...
def test_condition_gate_and_classification_are_fail_closed() -> None:
    ...
    assert core.classify_outcome({"alpha-050": False, "alpha-099": False}) == (
        "local_optimizer_or_jacobian_failure"
    )
    assert core.classify_outcome({"alpha-050": False, "alpha-099": True}) == "narrow_attraction_basin"
    assert core.classify_outcome({"alpha-050": True, "alpha-099": True}) == (
        "source_initialization_outside_usable_basin"
    )
    assert core.classify_outcome({"alpha-050": True, "alpha-099": False}) == "nonmonotone_stop_and_audit"


def test_formal_metrics_recompute_per_condition() -> None:
    config = core.load_config()
    for condition in ("alpha-050", "alpha-099"):
...
def test_formal_metrics_fail_closed(mutation: str) -> None:
    config = core.load_config()
    rows = [_metric(config, "alpha-050", step) for step in range(1, 257)]
    if mutation == "missing":
        rows.pop()
    elif mutation == "condition":
        rows[0]["condition"] = "alpha-099"
    elif mutation == "lr":
    ...
    with pytest.raises(ValueError):
        core.validate_metrics(rows, config, "alpha-050")
...
def test_runner_reuses_target_resets_optimizer_and_forbids_reader() -> None:
    ...
    assert "for condition in config[\"initialization_conditions\"]" in source
...
def _synthetic_delivery(root: Path, *, mode: str) -> dict:
    ...
    config["parent_reachable_failure"]["source_target_artifact_sha256"] = core.sha256_file(target_path)
    config["parent_reachable_failure"]["target_tensor_sha256"] = target["tensor_sha256"]
    target_record = {
    ...
    result_conditions = {}
    for condition in config["initialization_conditions"]:
        name = condition["name"]
