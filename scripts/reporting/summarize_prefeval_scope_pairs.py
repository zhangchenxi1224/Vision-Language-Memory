"""Plan13 paired outcomes, scope confusion and branch-margin diagnostics from frozen records only."""
...
def main():
    ...
    roots={'C13':a.output,'S13':a.output,'M':a.source}
    ...
    paired={};panels={};cohorts={};failures=[];counterfactual={};overwrite={};confusion={};stratified={}
    for arm,rr in reads.items():
        pp=defaultdict(list);cc=defaultdict(list);matrix=defaultdict(int)
        for key,row in rr.items():
            ...
            if q.get('kind')=='recovery' or panel=='recovery_coverage':
                ...
                literal=[scope for scope,value in state.items() if value is not None and raw.strip()==value]
                if literal:returned='literal:'+','.join(sorted(literal))
                elif raw.strip()=='no active preference':returned='absence'
                elif ok:returned='scorer_equivalent_gold'
                else:returned='unmatched_content'
                matrix[subset+'|'+q['scope']+'|'+returned]+=1
                if not ok:
                    raw=row['generation']['raw'];state=p['targets'][sid]['state']
        ...
        overwrite[arm]=oo
    for now,before in (('C13','M'),('S13','M'),('S13','C13')):
        pp=defaultdict(lambda:dict(total=0,rescued=0,regressed=0,both_correct=0,both_wrong=0))
        strata=defaultdict(lambda:dict(total=0,rescued=0,regressed=0,both_correct=0,both_wrong=0))
        for key,row in reads[now].items():
            ...
            sid,panel,_=key;item=row['query'];t=p['targets'][sid]
            scope=item.get('scope');changed='changed' if scope==t['changed_scope'] else 'untouched'
            groups=sorted({q['semantic_group'] for q in ev['targets'][sid]['mcq'] if q['scope']==scope})
            for group in ('K'+str(len(t['state'])),changed,*('semantic:'+g for g in groups)):
                ds=strata[panel+'|'+group];ds['total']+=1
                ds['rescued']+=int(u and not v);ds['regressed']+=int(v and not u)
                ds['both_correct']+=int(u and v);ds['both_wrong']+=int(not u and not v)
        paired[f'{now}_vs_{before}']=dict(pp)
        stratified[f'{now}_vs_{before}']=dict(strata)
    ce=defaultdict(list)
    ...
    branch_summary={k:dict(queries=len(rows),active_queries=sum(r['contrast']['active']>0 for r in rows),
        comparisons=sum(len(r['contrast']['comparisons']) for r in rows),
        active_comparisons=sum(r['contrast']['active'] for r in rows),
        mean_query_hinge=sum(r['contrast']['contrast_loss'] for r in rows)/len(rows),
        minimum_branch_margin=min((c['margin'] for r in rows for c in r['contrast']['comparisons']),default=None)) for k,rows in ce.items()}
    result=dict(scope_confusion=confusion,stratified_pairs=stratified,branch_margins=branch_summary,generation_panels=panels,paired=paired,recovery_cohorts=cohorts,counterfactual_pairs=counterfactual,
