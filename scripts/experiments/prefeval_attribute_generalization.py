"""Plan10: old-situation control versus new attribute application training."""
...
def jobs(t,step,arm,new_cases):
    ...
    for scope,value in sorted(t['state'].items()):
        ...
        case=new_cases[old['value_id']][spec['case']] if arm=='D' and spec['bank'] else old
...
def register(a):
    ...
    reg=dict(plan='prefeval-rgb-attribute-generalization-10',parent_registration_digest=digest(parent),
        ...
        budget=dict(updates=5120,gradient_forwards={'R':38400,'D':38400,'total':76800},generations=5320,
            ranking_decisions=2256,max_candidate_forwards=9024,endpoint_recovery_forwards=528),writer_updates=0,
        limitation='Fixed 40 observed endpoints; tests attribute application generalization, not a fresh benchmark holdout')
    s.write_frozen(DATA/'registration.json',reg);print(json.dumps(dict(registration_digest=digest(reg),budget=reg['budget'])))

def guard(a,reg):
    assert j.actual_host()==reg['runtime']['actual'];assert s.file_sha(a.source/'final-verified.json')==reg['parent_verified']
...
def train(a):
    ...
    for sid in assignment:
        ...
        for arm in (('R','D') if sorted(p['targets']).index(sid)%2==0 else ('D','R')):
...
def evaluate(a):
    ...
    endpoints={f'{arm}/{sid}':s.file_sha(a.output/'training'/arm/sid/'complete.json') for sid in p['targets'] for arm in ('R','D')}
    ...
    for sid in assignment:
        ...
        for arm in ('R','D'):
            ...
            for item0 in t['application_training']:
                case=old_cases[(item0['case_id'],item0['scope'],item0['value_id'])];item,_,_=q.xml_candidates(case,0);g=s.generate(reader,processor,image,item['query'],a.device)
                s.append(out/'reads.jsonl',dict(target=sid,condition=arm,panel='application_xml',query=item,png_sha=sha,generation=g,score=s.score_generation(g,{**item,'target_index':ord(item['target'][-10])-65},True),reader_query=item['query']));n+=1
