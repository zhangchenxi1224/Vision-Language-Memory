...
当前结论：官方FM公式、完整时间域、纯噪声初态、原生scheduler已成套实现并验证，真实实验使用官方Base原生28步推理。rank16 LoRA与后续全U-Net/CFG1实验尚未通过完整功能验收。最新31逻辑条件抽样对照的开发为1460/1510，单次280/360、连续RGB链390/480（7/16整链）、历史前缀895/960；四路原始记录与PNG已本地完整重算。实际6写30读CLI重放一致，但不能消除功能失败。全302格首步实测进一步发现raw训练条件与原生Base推理条件的差异，现已在新4H200上启动相同4832步预算的条件编码对照，尚未得到新训练效果。完整证据与限制见[实验结果](official-alignment-results-20260913/README.md)，当前进程信息见[续接记录](official-alignment-continuation-20260913.md)。以下早期记录按实验阶段保留。
