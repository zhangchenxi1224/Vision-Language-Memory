"""Locked contracts and independent audit for the R11_new reachable-basin control."""
...
PROTOCOL = "R11-New-Reachable-Basin-Control-Target01"
PREFIX = "vision_memory.r11-new-reachable-basin-control"
CONFIG_PATH = Path(__file__).resolve().parents[3] / "configs/experiments/r11_new_reachable_basin_control_target01.json"
CONFIG_BYTES_SHA256 = "4f5bf2d73bc2d7bd10f57f0d4797aef17e9c6f42585ecbe44436ba5368d30cf0"
CONFIG_CANONICAL_SHA256 = "ec7e760f8e3438eed3f343e936fe44a95171fb29e839b5c5f6895295715a7275"
TARGET_TENSOR_NAMES = (
...
def load_config() -> dict[str, Any]:
    require(sha256_file(CONFIG_PATH) == CONFIG_BYTES_SHA256, "Reachable-basin config byte hash drift.")
    value = json.loads(CONFIG_PATH.read_text(encoding="utf-8"))
    require(canonical_sha(value) == CONFIG_CANONICAL_SHA256, "Reachable-basin config canonical hash drift.")
    require(value.get("protocol") == PROTOCOL and value.get("schema") == f"{PREFIX}-config.v1",
            "Reachable-basin config identity drift.")
    conditions = value.get("initialization_conditions")
    require(conditions == [
        {"name": "alpha-050", "alpha": 0.5,
         "formula": "source_x_T + 0.50 * (teacher_x_T - source_x_T)"},
        {"name": "alpha-099", "alpha": 0.99,
         "formula": "source_x_T + 0.99 * (teacher_x_T - source_x_T)"},
    ], "Warm-start conditions drift.")
    require(value["fixed_target_contract"]["regenerate_noise_or_target_forbidden"] is True,
...
def condition_map(config: Mapping[str, Any]) -> dict[str, Mapping[str, Any]]:
    conditions = {item["name"]: item for item in config["initialization_conditions"]}
    require(tuple(conditions) == ("alpha-050", "alpha-099"), "Condition order drift.")
    return conditions
...
def per_condition_gate(distance: Mapping[str, Any], *, technical_gate: bool,
    ...
    require(type(technical_gate) is bool and all(type(value) in (int, float) and math.isfinite(value)
            and value >= 0.0 for value in values.values()), "Invalid basin-gate inputs.")
...
def classify_outcome(gates: Mapping[str, Any]) -> str:
    alpha050 = gates.get("alpha-050")
    alpha099 = gates.get("alpha-099")
    require(type(alpha050) is bool and type(alpha099) is bool, "Missing condition gates.")
    if alpha050 and not alpha099:
        return "nonmonotone_stop_and_audit"
    if not alpha099:
        return "local_optimizer_or_jacobian_failure"
    if not alpha050:
        return "narrow_attraction_basin"
    return "source_initialization_outside_usable_basin"


def validate_metrics(rows: Sequence[Mapping[str, Any]], config: Mapping[str, Any],
                     condition_name: str) -> dict[str, Any]:
    expected_steps = int(config["unchanged_optimizer_contract"]["optimizer_steps_per_condition"])
...
def validate_fixed_target(root: Path, result: Mapping[str, Any], config: Mapping[str, Any]) -> dict[str, Any]:
    ...
    parent = config["parent_reachable_failure"]
...
def _validate_checkpoint(path: Path, *, condition_name: str, step: int,
                         target: Mapping[str, torch.Tensor], alpha: float,
...
def audit_delivery(root: Path, config: Mapping[str, Any]) -> dict[str, Any]:
    ...
    require(required.issubset(listed), "Required basin-control artifacts missing.")
    ...
    require(tuple(result_conditions) == tuple(conditions), "Result condition order drift.")
    for name, condition in conditions.items():
        relative = f"conditions/{name}/checkpoints/step-000.pt"
        require(relative in listed, f"Missing initial checkpoint: {name}.")
        _, initial_distance = _validate_checkpoint(
            root / relative, condition_name=name, step=0, target=target, alpha=float(condition["alpha"])
        )
        record = result_conditions[name]
        require(record.get("alpha") == condition["alpha"]
    ...
    if mode == "technical-preflight":
        ...
        require(prerequisite.get("passed") is True and prerequisite.get("mode") == "technical-preflight"
                ...
                == config["parent_reachable_failure"]["source_target_artifact_sha256"]
                and prerequisite.get("target_tensor_sha256")
                == config["parent_reachable_failure"]["target_tensor_sha256"],
        ...
        for name, condition in conditions.items():
            ...
            for step in config["unchanged_optimizer_contract"]["checkpoint_steps"]:
                ...
                _, final_distance = _validate_checkpoint(
                    ...
                    alpha=float(condition["alpha"]), m0_mse=m0_mse
                )
                checkpoint_steps.append(step)
            require(checkpoint_steps == config["unchanged_optimizer_contract"]["checkpoint_steps"]
