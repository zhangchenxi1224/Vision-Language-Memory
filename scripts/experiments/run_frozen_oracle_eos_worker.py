...
SCHEMA = "vision_memory.frozen-oracle-eos-worker.v1"
STEPS = 256
...
def strict_endpoint_gate(rows):
    from vision_memory.training.frozen_oracle_eos import endpoint_gate
    return endpoint_gate(rows)
...
def _load_runtime(args: argparse.Namespace) -> dict[str, Any]:
    ...
    from vision_memory.training.frozen_oracle_eos import question_prompts
    prompts = question_prompts(target.query.text)
    termination = assistant_termination_contract(reader, processor)
    gold = target.query.choices[target.query.target_index]
    donor_spec = args.config_payload["donors"][str(args.target_index)]
    donor_path = Path(donor_spec["path"])
    if file_sha256(donor_path) != donor_spec['sha256']:
        raise RuntimeError('Preregistered donor content changed')
    donor_array = torch.from_numpy(np.load(donor_path, allow_pickle=False)).to(source)
    if donor_spec["gold"] == gold or donor_array.shape != source.shape:
        raise RuntimeError("Donor must bind a different answer with the same latent shape")
    with torch.no_grad():
        reset_image = legacy.decode_model_latents_unit_interval(pipe.vae, source, clamp=True)
        donor_image = legacy.decode_model_latents_unit_interval(pipe.vae, donor_array, clamp=True)
    runtime = dict(legacy=legacy, oracle=oracle, reader=reader, pipe=pipe, target=target, source=source,
                   processor=processor, reader_device=reader_device, prompts=prompts, gold=gold,
                   termination=termination, condition=condition, reset_image=reset_image,
                   donor_image=donor_image, donor_binding={**donor_spec, "sha256": file_sha256(donor_path)},
...
def _manifest(args: argparse.Namespace, runtime: Mapping[str, Any]) -> dict[str, Any]:
    ...
    result = {
        ...
        "open_prompts": runtime["prompts"], "gold": runtime["gold"], "termination_contract": runtime["termination"],
        "donor_binding": runtime["donor_binding"],
        ...
        "contract": {"optimizer": "Adam", "optimizer_steps": STEPS, "lr": LEARNING_RATE,
                     ...
                     "training_views": "original_open only; no choices", "gold_eos_appended": True,
                     "objective": "mean_answer_ce + eos_ce", "lambda_eos": 1.0,
                     "endpoint_views": "original + four question-only paraphrases; identical two instruction lines",
                     "success": "original_open raw greedy32 exact match at step256", "legacy_gate": "not reused"},
    }
    write_json(out / "manifest.json", result)
    return result


def _teacher(runtime, image, prompt_id="original_open", require_grad=False):
    from vision_memory.reader.open_eos import qwen3vl_answer_eos_ce
    from vision_memory.reader.qwen3vl import R3_QWEN_READER_RESIZE_CONTRACT
    return qwen3vl_answer_eos_ce(model=runtime["reader"], processor=runtime["processor"],
        image=image[0].to(runtime["reader_device"]), device=runtime["reader_device"],
        query=runtime["prompts"][prompt_id], target=runtime["gold"],
        termination=runtime["termination"], lambda_eos=1.0, require_image_grad=require_grad,
        reader_resize_contract=R3_QWEN_READER_RESIZE_CONTRACT, deterministic_ce=True)


def _reader_forward(runtime: Mapping[str, Any], image: torch.Tensor, step: int):
    output = _teacher(runtime, image, require_grad=True)
    scores = {"ce": float(output.loss.detach()), "answer_ce": float(output.answer_loss.detach()),
              "eos_ce": float(output.eos_loss.detach()), "lambda_eos": 1.,
              "answer_token_count": output.answer_token_count,
              "gold_eos_appended": True, "prompt_id": "original_open"}
    return output, scores


@torch.no_grad()
def _evaluate_image(runtime: Mapping[str, Any], image: torch.Tensor):
    ...
    rows = []
    for condition, pixels in (("matched", image), ("blank", runtime["reset_image"]), ("donor", runtime["donor_image"])):
        for prompt_id, query in runtime["prompts"].items():
            output = _teacher(runtime, pixels, prompt_id)
            ...
            rows.append({"condition": condition, "prompt_id": prompt_id, "query": query,
                "gold": runtime["gold"], "image_sha256": canonical_tensor_sha256(pixels),
                "answer_ce": float(output.answer_loss), "eos_ce": float(output.eos_loss),
                "ce": float(output.loss), "gold_eos_appended": True,
                **generated, **generation_diagnostics(generated, runtime["gold"], gold_ids)})
    return rows
...
def _optimize(args: argparse.Namespace, runtime: Mapping[str, Any]) -> dict[str, Any]:
    ...
    return {**args.spec, "schema": SCHEMA, "status": "completed", "mode": "optimize",
            ...
            "legacy_reachability_gate": None, "legacy_reachability_statistics": None,
            "training_objective": "mean_answer_ce + eos_ce", "training_prompt": "original_open",
            "gold_eos_appended": True, "choice_forward_calls": 0,
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    try:
        ...
        if file_sha256(Path(runtime["donor_binding"]["path"])) != runtime["donor_binding"]["sha256"]:
            raise RuntimeError("Donor file changed during this run")
