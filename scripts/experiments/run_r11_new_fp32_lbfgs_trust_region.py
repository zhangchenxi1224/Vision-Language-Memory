"""Run the preregistered matched-budget FP32 L-BFGS trust-region control."""
...
from vision_memory.training import r11_new_fp32_prpplus_trust_region as prp_core  # noqa: E402
...
def validate_environment(args: Any, config: Mapping[str, Any]) -> dict[str, Any]:
    ...
    for name, (path, expected) in bindings.items():
        core.require(path.is_file() and core.sha256_file(path) == expected, f"L-BFGS parent {name} binding drift.")
    parent_config = prp_core.load_config()
    parent_audit = prp_core.audit_delivery(parent_root, parent_config)
    core.require(
        ...
        "Wrong L-BFGS parent PRP+ result.",
    )
    return {
        **validation,
        "parent_prpplus_audit": parent_audit,
        "parent_prpplus_bindings": {
    ...
    *,
    ...
    with (
        ...
        for iteration in range(maximum_iterations):
            evaluated_x = current_x.clone()
            parent_runner._set_student(oracle, evaluated_x)
            ...
            if expected_current_endpoint is None:
                ...
                core.require(
                    ...
                    "L-BFGS current endpoint replay drift.",
                )
            target = teacher_endpoint.to(output.z_t.device, dtype=torch.float32)
            loss = (output.z_t.float() - target).square().mean()
            core.require(loss.numel() == 1 and bool(torch.isfinite(loss)), "Invalid L-BFGS loss.")
            loss.backward()
            ...
            core.require(
                ...
                "L-BFGS gradient density gate failed.",
            )
            pair_state = core.form_curvature_pair(
                iteration=iteration,
                current_x=evaluated_x,
            ...
            if pair_state["accepted"]:
                ...
                history = history[-core.HISTORY_SIZE :]
            direction_state = core.lbfgs_direction(gradient_cpu, history, iteration=iteration)
            raw_direction = direction_state["raw_direction"]
            direction = direction_state["direction"]
            if direction_state["history_cleared"]:
                history = []
            oracle.x_T_fp32.grad = None
            ...
            for radius_index, radius in enumerate(core.RADII):
                candidate_x = evaluated_x + radius * direction
                ...
                print(
                    f"lbfgs iteration={iteration} candidate={radius_index} "
            ...
            curvature_metadata = core.curvature_row_metadata(pair_state)
            direction_metadata = core.direction_row_metadata(direction_state)
            iteration_row = {
                ...
                "current_x_T_fp32_sha256": canonical_tensor_sha256(evaluated_x),
            ...
            tensor_iterations[iteration] = {
                "current_x_T_fp32": evaluated_x,
                "current_endpoint_fp32": current_endpoint,
                "gradient_fp32": gradient_cpu,
                "curvature_s_fp32": pair_state["s"],
                "curvature_y_fp32": pair_state["y"],
                "inverse_hessian_raw_direction_fp32": raw_direction,
                "negative_gradient_direction_fp32": direction,
                "history_pair_iterations_after_direction": direction_state["history_pair_iterations_after_direction"],
            ...
            if accepted:
                ...
                core.require(best_image is not None, "Missing accepted L-BFGS image.")
                phase1a._save_image(
                    args.output_root / f"accepted_images/{core.iteration_id(iteration)}.png",
                    best_image,
                )
            previous_x = evaluated_x
            previous_gradient = gradient_cpu.clone()
            print(
                f"lbfgs selected iteration={iteration} history={direction_metadata['history_size_used']} "
            ...
            if not accepted or best["loss_ratio_to_plateau"] <= success_ratio:
                break
    core.require(expected_current_endpoint is not None, "L-BFGS failed before the first endpoint evaluation.")
...
def write_report(root: Path, result: Mapping[str, Any]) -> None:
    ...
    lines = [
        "# R11_new FP32 L-BFGS trust-region control",
        "",
        f"- Mode: `{result['mode']}`",
        ...
        f"- Curvature pairs: `{json.dumps(summary['curvature_pair_counts'], sort_keys=True)}`",
        f"- Maximum history size: `{summary['maximum_history_size_used']}`",
