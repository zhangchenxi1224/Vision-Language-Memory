...
def _build_turns(
    *,
    ...
    if transition_profile == "set_only":
        ...
        raise ValueError(f"transition_profile must be one of {TRANSITION_PROFILES}, got {transition_profile!r}")
        ...
        if semantic_variant == 0:
            transition = event(EventKind.CLEAR, None)
            intermediate = NO_ACTIVE_PREFERENCE
        else:
            # The active counterfactual deliberately mirrors the clear path's
            # update/read schedule.  Across semantic variants x q0/q1 the four
            # shared visible candidates are each the target exactly once.
            intermediate = matched_values[2]
            transition = event(EventKind.OVERWRITE, intermediate)
        final_kind = EventKind.SET if semantic_variant == 0 else EventKind.OVERWRITE
        query_intermediate = Turn(
            TurnType.QUERY,
            query=_query(
                entity,
                topic,
        ...
        if include_distractors:
            turns.append(noop)
        turns.extend((transition, query_intermediate, final_mixed))
    else:
...
def _canonicalize_full_profile_choices(
    episodes: list[Episode],
    *,
    seed: int,
) -> list[Episode]:
    """Give every visible payload one deterministic order and four balanced targets."""

    references: dict[tuple[str, tuple[str, ...]], list[tuple[int, int]]] = defaultdict(list)
    for episode_index, episode in enumerate(episodes):
        for turn_index, turn in enumerate(episode.turns):
            ...
            references[key].append((episode_index, turn_index))

    ordered_by_key: dict[tuple[str, tuple[str, ...]], tuple[str, str, str, str]] = {}
    for key, items in references.items():
        _, candidates = key
        target_counts: Counter[str] = Counter(
            episodes[episode_index].turns[turn_index].query.target for episode_index, turn_index in items
        )
        if set(target_counts) != set(candidates) or any(count * 4 != len(items) for count in target_counts.values()):
            raise ValueError(
                "Full-profile canonical query payloads must expose all four targets "
                f"with share 0.25, got {dict(target_counts)} for {key!r}"
            )
        payload = json.dumps(
            [seed, key[0], list(candidates)],
            ensure_ascii=False,
            separators=(",", ":"),
        )
        order_seed = int.from_bytes(hashlib.sha256(payload.encode()).digest()[:8], "big")
        ordered = list(candidates)
        random.Random(order_seed).shuffle(ordered)
        ordered_by_key[key] = tuple(ordered)  # type: ignore[assignment]

    canonicalized: list[Episode] = []
    for episode in episodes:
        ...
        for turn in episode.turns:
            if turn.query is None:
                ...
                continue
            query = turn.query
            key = (query.text, tuple(sorted(query.choices)))
            choices = ordered_by_key[key]
            updated_query = replace(
                query,
                choices=choices,
                target_index=choices.index(query.target),
            )
            turns.append(replace(turn, query=updated_query))
        canonicalized.append(replace(episode, turns=tuple(turns)))
    return canonicalized


def _covariate_index(
    group_number: int,
    size: int,
    *,
    block_stride: int,
    ...
    """Latin-style schedule that does not alias a covariate to pattern=group%4."""

    block, pattern = divmod(group_number, 4)
    return (pattern + block * block_stride + offset) % size
...
def _rebalance_target_positions(episodes: list[Episode], *, seed: int) -> list[Episode]:
    ...
    for comparison_id, items in groups:
        weight = len(items)
        candidates = [position for position in range(4) if position_counts[position] + weight <= target_per_position]
        if not candidates:
    ...
    *,
    ...
    for ood_group, count in segments:
        ...
        while remaining:
            ...
            split_offset = global_seed + sum(map(ord, split))
            entity_families = HELDOUT_ENTITY_FAMILIES if ood_group == "heldout_entity" else BASE_ENTITY_FAMILIES
            ...
            family_bank = (
                PARAPHRASE_TEMPLATE_FAMILIES if ood_group == "heldout_paraphrase" else SPLIT_TEMPLATE_FAMILIES[split]
            )
            template_family = family_bank[
                _covariate_index(
                    group_number,
                    len(family_bank),
                    block_stride=3,
            ...
            template_id = f"{split}-{ood_group or 'id'}-{_slug(template_family)}-pattern-{pattern}-style-{style % 3}"
            ...
            topic = topic_names[
                _covariate_index(
                    ...
                    len(topic_names),
                    block_stride=5,
                    offset=split_offset + 3,
                )
            ]
            values = topic_bank[topic]
            pair_rng = random.Random(global_seed * 1_000_003 + group_number + sum(map(ord, split)))
            selected = pair_rng.sample(values, 4)
            if transition_profile == "full" and pattern == 2:
                matched_values = (
            ...
            for semantic_variant in range(2):
                for stream_variant in stream_variants:
                    ...
                    semantic_seed = global_seed * 10_000_019 + group_number * 2 + semantic_variant
                    if transition_profile == "full" and pattern == 2:
                        # sem0: SET active1 -> CLEAR -> N/A -> SET active0
                        # sem1: SET active0 -> OVERWRITE active1 -> SET active2
                        initial = selected[1] if semantic_variant == 0 else selected[0]
                        final = selected[0] if semantic_variant == 0 else selected[2]
                    else:
                        initial = selected[semantic_variant]
                        final = selected[semantic_variant + 2]
                    comparison_prefix = f"{split}-stream-{group_number:06d}-semantic-{semantic_variant}"
                    turns = _build_turns(
                        entity=entity,
                        topic=topic,
                        initial=initial,
                        ...
                        matched_values=matched_values,  # type: ignore[arg-type]
                    ...
                    if complete_distractor_grid:
                        ...
                        distractor_pair_id = f"{split}-stream-{group_number:06d}-semantic-{semantic_variant}"
                    else:
                    ...
                    episodes.append(
                        Episode(
                            ...
                            template_family=template_family,
                            turns=turns,
                            pair_id=(f"{split}-counterfactual-{group_number:06d}-{stream_variant.value}"),
    ...
    if transition_profile == "full":
        return _canonicalize_full_profile_choices(
            episodes,
            seed=global_seed + sum(map(ord, split)),
        )
...
def _surface_partition(episodes: list[Episode]) -> dict[str, Any]:
    ...
    variants = Counter(
        episode.distractor_variant.value for episode in episodes if episode.distractor_variant is not None
    )
    return {
