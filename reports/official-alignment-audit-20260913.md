...
当前结论：官方FM公式、完整时间域、纯噪声初态、原生scheduler与条件接口已成套实现并验证，真实实验使用官方Base原生28步推理。rank16 LoRA在512和3500步均未通过；后续采用全U-Net和显式native CFG1，根据原始失败证据扩大状态、source和事件表达覆盖。已完成的45条件四H200训练开发集900/900，但独立单图340/360、连续RGB链410/480（9/16整链），因此仍未得到通过功能验收的版本。其独立推理包实际6写30读重放一致，不能消除功能失败。当前151条件/17语义题实验保持官方FM与原生采样不变，增加事件表达和已验证的16个历史目标，在同一用户四H200实例进行固定4832步训练，后接预注册独立验证。完整证据与限制见[实验结果](official-alignment-results-20260913/README.md)，当前进程信息见[续接记录](official-alignment-continuation-20260913.md)。以下早期记录按实验阶段保留。
