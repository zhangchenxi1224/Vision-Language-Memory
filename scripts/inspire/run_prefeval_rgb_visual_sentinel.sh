...
"$task_python" scripts/reporting/verify_prefeval_rgb_references.py --directory "$task_run/references-v2" \
  --overlay reports/prefeval-rgb-20260917/reader-format-v2.json --output "$task_run/references-v2-verified.json" || exit 1
"$task_python" -c 'import json,sys; assert json.load(open(sys.argv[1]))["gate_passed"], "Reference gate failed"' "$task_run/references-v2-verified.json" || exit 1
for phase in baseline sentinel; do
  output="$task_run/$phase-v2"
  mkdir -p "$output"
