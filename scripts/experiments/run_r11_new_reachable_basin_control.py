"""Run or independently audit the preregistered R11_new reachable-basin control."""
...
def validate_environment(args: argparse.Namespace, config: Mapping[str, Any]) -> dict[str, Any]:
    ...
    parent_archive = parent_root.parent / "r11-new-reachable-986a181-20260907-round01.tar.gz"
    bindings = {
        "target": (parent_root / "target/reachable_target.pt", parent["source_target_artifact_sha256"]),
...
def save_checkpoint(root: Path, *, condition_name: str, alpha: float, step: int,
...
def load_fixed_runtime(args: argparse.Namespace, config: Mapping[str, Any], counters: dict[str, int]):
    ...
    parent_path = Path(config["parent_reachable_failure"]["source_target_path"])
    ...
    core.require(target.get("tensor_sha256") == config["parent_reachable_failure"]["target_tensor_sha256"],
    ...
    target_record = {
        ...
        "source_parent_root": config["parent_reachable_failure"]["source_root"],
    }
...
def train_condition(*, args: argparse.Namespace, config: Mapping[str, Any], condition: Mapping[str, Any],
    ...
    checkpoints = [save_checkpoint(
        args.output_root, condition_name=name, alpha=alpha, step=0, oracle=oracle, optimizer=optimizer,
    ...
    if args.mode == "technical-preflight":
        ...
        return {"alpha": alpha, "m0_mse": m0_mse, "gradient_probe": probe,
    ...
    with metrics_path.open("x", encoding="utf-8", newline="\n") as metrics:
        for update in range(1, config["unchanged_optimizer_contract"]["optimizer_steps_per_condition"] + 1):
            learning_rate = core.optimizer_learning_rate(update, config)
            ...
            if update in config["unchanged_optimizer_contract"]["checkpoint_steps"][1:]:
                ...
                checkpoints.append(save_checkpoint(
                    args.output_root, condition_name=name, alpha=alpha, step=update, oracle=oracle,
...
def write_report(root: Path, result: Mapping[str, Any]) -> None:
    lines = [
        "# R11_new reachable-basin control",
        "",
        f"- Mode: `{result['mode']}`",
...
def run_experiment(args: argparse.Namespace, config: Mapping[str, Any], validation: Mapping[str, Any]) -> dict[str, Any]:
    ...
    def forbidden_reader(_module: Any, _inputs: Any) -> None:
        ...
        raise RuntimeError("Reader is forbidden in the numerical reachable-basin control.")
    ...
    try:
        ...
        manifest = {
            ...
            "condition_order": config["unchanged_optimizer_contract"]["condition_order"],
        ...
        if args.mode == "technical-preflight":
            ...
            technical_gate = bool(
                ...
                and all([item["optimizer_step"] for item in record["checkpoint_records"]]
                        == config["unchanged_optimizer_contract"]["checkpoint_steps"]
                        for record in condition_results.values())
                and models_frozen(oracle, reader) and only_student_trainable(oracle, reader)
            )
...
def main(argv: list[str] | None = None) -> int:
    ...
    lock_path = Path("/tmp/vision-memory-r11-new-reachable-basin-control.lock")
