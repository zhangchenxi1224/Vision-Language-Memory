"""R13 mean-centered event residual writer with fixed causal evaluation.

The R12 endpoint is decomposed exactly into a frozen common visual base plus an
event-conditioned residual.  Only the residual branch and its latent basis are
trainable, and the residual coefficient mean over the fixed 144 training events
is subtracted on every forward pass.  This makes an event-independent learned
visual update structurally unavailable while preserving the R12 initialization.
"""
...
IMPLEMENTATION_REVISION = "r12-exact-decomposition-centered-residual-v1"
MANIFEST_SCHEMA = "vision_memory.r13-centered-residual-writer-manifest.v1"
MICRO_SCHEMA = "vision_memory.r13-centered-residual-writer-micro.v1"
OPTIMIZER_SCHEMA = "vision_memory.r13-centered-residual-writer-optimizer.v1"
SUMMARY_SCHEMA = "vision_memory.r13-centered-residual-writer-summary.v1"
TECHNICAL_GATE_SCHEMA = "vision_memory.r13-centered-residual-writer-technical-gate.v1"
EMBEDDING_AUDIT_SCHEMA = "vision_memory.r13-event-embedding-cache.v1"
SOURCE_AUDIT_SCHEMA = "vision_memory.r13-r12-source-decomposition.v1"
SUITE_PREFIX = "r13_f1_centered_residual"
CONFIG_PATH = ROOT / "configs" / "experiments" / "r13_centered_residual_writer.json"
...
def parse_args(argv: Sequence[str] | None = None) -> argparse.Namespace:
    ...
    if args.seed != 0:
        raise ValueError("R13 fixes seed=0.")
    args.adapter_seed = 0
    args.pairing_seed = 0
...
def _load_json(path: Path) -> dict[str, Any]:
    ...
    if not isinstance(value, dict):
        raise ValueError(f"R13 expected a JSON object: {path}")
    return value
...
def _validate_source_arm(*, root: Path, arm: str, expected: Mapping[str, str]) -> dict[str, Path]:
    ...
    for name, path in paths.items():
        if not path.is_file():
            raise ValueError(f"R13 missing R12 {arm} source {name}: {path}")
    observed = {name: _sha256(paths[name]) for name in expected}
    if observed != dict(expected):
        raise ValueError(f"R13 R12 {arm} source hash drift: {observed}")
    ...
    if (
        ...
        raise ValueError(f"R13 requires the completed failed-science/valid-technical R12 {arm} arm.")
    return paths


def _validate_args(args: argparse.Namespace) -> dict[str, Any]:
    if not torch.cuda.is_available():
        raise RuntimeError("R13 requires CUDA.")
    if args.dreamlite_device == args.reader_device:
        raise ValueError("R13 writer/VAE and Reader must use distinct devices.")
    for name in ("train", "dev", "r12_comparison", "r12_collapse_audit"):
        if not getattr(args, name).is_file():
            raise ValueError(f"R13 missing file argument {name}: {getattr(args, name)}")
    for name in ("dreamlite", "reader", "r12_conditioned_root", "r12_control_root"):
        if not getattr(args, name).is_dir():
            raise ValueError(f"R13 missing directory argument {name}: {getattr(args, name)}")
    if args.output_dir.exists() and any(args.output_dir.iterdir()):
        raise ValueError("R13 refuses a non-empty output directory.")
    config = _load_json(CONFIG_PATH)
    if (
        config.get("schema") != "vision_memory.r13-centered-residual-writer-config.v1"
        or config.get("status") != "preregistered_before_any_r13_model_outcome"
    ):
        raise ValueError("R13 configuration is not in the locked preregistered state.")
    head = r5.git_value("rev-parse", "HEAD")
    dirty = bool(r5.git_value("status", "--porcelain"))
    if dirty and not args.allow_dirty:
        raise ValueError("R13 requires a clean source snapshot.")
    ...
    if observed_data != {
        ...
        raise ValueError(f"R13 formal data hash drift: {observed_data}")
    parent = config["activation"]
    if (
        not SOURCE_PREREG_PATH.is_file()
        or _sha256(SOURCE_PREREG_PATH) != parent["source_decomposition_preregistration_sha256"]
    ):
        raise ValueError("R13 source-decomposition preregistration artifact drifted.")
    ...
    if (
        ...
        raise ValueError("R13 source-decomposition preregistration content drifted.")
    if _sha256(args.r12_comparison) != parent["r12_comparison_sha256"]:
        raise ValueError("R13 comparison parent hash drifted.")
    if _sha256(args.r12_collapse_audit) != parent["r12_collapse_audit_sha256"]:
        raise ValueError("R13 collapse-audit parent hash drifted.")
    ...
    return {
        ...
        "source_decomposition_preregistration_sha256": _sha256(SOURCE_PREREG_PATH),
    }
...
def _load_checkpoint_state(path: Path, *, expected_arm: str) -> Mapping[str, Tensor]:
    ...
    if (
        ...
        raise ValueError(f"R13 invalid R12 {expected_arm} step-1152 checkpoint.")
    return state
...
def _embedding_cache(
    *,
    ...
    if (
        ...
        raise ValueError("R13 invalid R12 source embedding cache.")
    ...
    with torch.no_grad():
        for start in range(0, len(ordered), batch_size):
            ...
            for index, segment in enumerate(batch):
                ...
                if length <= 0:
                    raise RuntimeError("R13 event encoder produced an empty sequence.")
                ...
                if value.ndim != 2 or value.shape[1] != 2048 or not torch.isfinite(value).all():
                    raise RuntimeError("R13 event encoder produced invalid token states.")
                source_value = source_cache.get(segment.segment_id)
                matches = source_value is None or torch.equal(value, source_value)
                if not matches:
                    raise RuntimeError(f"R13 event embedding drift for {segment.segment_id}.")
...
def _pooled_features(
    ...
    for segment_id in sorted(cache):
        ...
        if pooled.shape != (2048,) or not torch.isfinite(pooled).all():
            raise RuntimeError(f"R13 invalid pooled feature for {segment_id}.")
...
class MeanCenteredResidualWriter(nn.Module):
    ...
    def __init__(
        ...
        *,
        ...
        if tuple(initial_latent.shape) != (1, 4, 128, 128):
            raise ValueError("R13 initial latent shape drifted.")
        if train_features.shape != (R12_TRAIN_SEGMENT_COUNT, 2048):
            raise ValueError(f"R13 train feature shape drifted: {tuple(train_features.shape)}")
    ...
    def forward(self, feature: Tensor, *, conditioned: bool = True) -> tuple[Tensor, dict[str, Tensor]]:
        if feature.ndim != 2 or feature.shape != (1, 2048):
            raise ValueError(f"R13 feature shape drifted: {tuple(feature.shape)}")
...
def _build_source_decomposition(
    *,
    ...
    if base_hash != expected_base_hash:
        raise RuntimeError(f"R13 fixed-base hash drift: expected {expected_base_hash}, observed {base_hash}.")
    if max_error > float(config["technical_gate"]["source_reconstruction_max_abs_lte"]):
        raise RuntimeError(f"R13 source decomposition reconstruction drift: {max_error}")
    audit = {
...
def _gradient_statistics(writer: MeanCenteredResidualWriter) -> dict[str, Any]:
    ...
    for name, parameter in writer.named_parameters():
        ...
        if not torch.isfinite(gradient).all():
            raise RuntimeError(f"R13 non-finite gradient in {name}.")
        active.append(name)
    ...
    if not math.isfinite(norm) or norm <= 0.0 or nonzero <= 0:
        raise RuntimeError("R13 produced a non-finite or zero aggregate gradient.")
    return {
        "gradient_norm": norm,
...
def _save_checkpoint(
    *,
    ...
    save_training_checkpoint(
        ...
        manifest=manifest,
        trainer_state={
            "schema": "vision_memory.r13-centered-residual-checkpoint-state.v1",
...
def _manifest(
    *,
    ...
    return {
        ...
        "primary_endpoint": f"centered_step{R12_OPTIMIZER_STEPS}",
    ...
    *,
    ...
    evaluation_complete = all(
        ...
        == {
            ...
            for checkpoint in ("m0", f"centered_step{R12_OPTIMIZER_STEPS}")
            for condition in ("normal", "reset", "donor", "base")
            for view in range(4)
        }
    ...
    checks = {
        ...
        "finite_metrics": all(
            math.isfinite(float(row[key])) for row in micro_rows for key in ("ce", "objective", "residual_rms")
        ),
        ...
        and embedding_audit.get("fresh_segment_count") == R13_FRESH_DEV_FINAL_COUNT,
...
def _run(args: argparse.Namespace) -> dict[str, Any]:
    ...
    if selection_payload["dev_final"]["payload_sha256"] != R13_FRESH_DEV_FINAL_SHA256:
        raise RuntimeError("R13 fresh-final payload hash drifted after selection.")
    relationship_audit = _selection_relationship_audit(selections)
    if not relationship_audit["passed"]:
        raise RuntimeError(f"R13 split relationship audit failed: {relationship_audit}")
    _write_json(
    ...
    if canonical_tensor_sha256(initial_latent) != INITIAL_LATENT_SHA256:
        raise RuntimeError("R13 blank latent hash drifted.")
    ...
    for unit in schedule:
        ...
        output = _reader_output(train_reader, image=image, segment=segment, permutation=permutation)
        ce = output.loss
        if not isinstance(ce, Tensor) or ce.numel() != 1 or not torch.isfinite(ce):
            raise RuntimeError("R13 Reader returned an invalid scalar CE.")
        residual_penalty = _latent_rms_penalty(diagnostics["residual"])
        coefficient_penalty = R12_COEFFICIENT_L2_PENALTY * diagnostics["residual_coefficients"].square().mean()
        objective = ce + residual_penalty.to(device=ce.device) + coefficient_penalty.to(device=ce.device)
        if not torch.isfinite(objective):
            raise RuntimeError("R13 objective became non-finite.")
    ...
    endpoint = f"centered_step{R12_OPTIMIZER_STEPS}"
    ...
    required = {
        ...
        "dev_final": R13_FRESH_DEV_FINAL_COUNT,
    }
    arm_gate = bool(technical["passed"] and pass_counts == required)
    if arm_gate:
        decision = "centered_shared_f1_candidate_pass_advance_to_recurrent_state_algebra"
    elif pass_counts["train_audit"] == R12_TRAIN_AUDIT_COUNT:
        decision = "centered_writer_fit_without_complete_heldout_causal_generalization"
    else:
        decision = "centered_residual_writer_did_not_fit_fixed_f1_boundary"
    summary = {
    ...
    _write_json(args.output_dir / "r13_centered_residual_summary.json", summary)
