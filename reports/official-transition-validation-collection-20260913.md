...
`scripts/inspire/run_transition_validation_suite.sh` now provides the executable sequence for a fresh idle H200 after the fixed parent has completed. Its first argument is an explicit Unix deadline inside the actual notebook lease, with at least90min remaining; optional `--diagnostic` is required to continue a failed development gate. It binds the already deployed probe0f40767, collector3c335a2 and package/parity281b653 checkouts and refuses dirty source, active GPU processes or existing outputs.

The sequence runs single writes and their full collector, RGB chains and their full collector plus96-tensor audit, new-endpoint parameter export, preparation of the first preregistered chain, and independent bank-free CLI replay and parity verification. Any command failure stops the sequence and preserves the failed stage. Workload completion is explicitly separate from functional correctness. The CLI replay is also bounded by the remaining deadline. This launcher passed `bash -n`; it has **not been dispatched** while the parent is training. Its fixed result names begin `0f40767-transition-` or `281b653-transition-`; it does not overwrite earlier failures.
