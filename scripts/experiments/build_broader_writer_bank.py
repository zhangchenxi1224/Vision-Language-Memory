...
REFINER_COMMIT = '685d65792b3c63e199a4558800113cfbfbe0d712'
...
def build(transition_path, historical_path, refinement, output, expected_commit):
    import torch
    ...
    if (read(plan_path) != plan(original) or sha(plan_path) != complete['plan_sha256']
    ...
    if sha(refined_path) != complete['bank_manifest_sha256']:
    ...
    if loaded['groups'][:45] != transition['groups'] or len(tensors) != 61 or torch.cuda.is_initialized():
        raise ValueError('Merge changed old conditions or failed CPU verification')
    seal = {'manifest_sha256': sha(manifest), 'conditional_groups': 61, 'semantic_questions': 17,
        'unchanged_transition_groups': 45, 'historical_groups': 16, 'teacher_records': 61,
