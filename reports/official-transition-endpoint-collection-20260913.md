...
`scripts/reporting/transition_state_geometry.py --run RUN --bank BANK --output NEW_JSON` adds a CPU diagnostic on the completed endpoint. It requires all180 generated tensors and all900 matched raw answers, verifies the phase seal, teacher file/tensor hashes and each generated tensor/Reader image binding, and compares every output latent with the three unchanged teacher states. It retains source, operation, event wording, noise, all raw answers and exact EOS scores. The summary separates failed images nearest the expected teacher from failures nearest a different teacher; latent distance does not establish semantic correctness or prove a causal mechanism. The tool does not select images for further training or alter any scoring. Its CLI has been checked; actual endpoint analysis is pending completion of the full final phase.
