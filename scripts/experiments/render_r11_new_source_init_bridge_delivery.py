"""Render the source-only initialization diagnostic from verified raw evidence.
...
from scripts.experiments import compare_r11_new_source_init_bridge as aggregate  # noqa: E402
from vision_memory.repro import canonical_object_sha256, canonical_tensor_sha256  # noqa: E402


core = aggregate.core
STEPS = (0, 64, 128, 192, 256)
SCHEMA = "vision_memory.r11-new-bridge-source-init-delivery.v1"
...
def collect_evidence(
    *, preflight_root: Path, formal_root: Path, aggregation_root: Path,
    ...
    required = {
        ...
        "primary_endpoint": core.BRIDGE_PRIMARY_ENDPOINT, "initialization_teacher_assisted": False,
    ...
    for label, root in (("preflight", preflight_run), ("formal", run)):
        ...
        _require(initialization.get("passed") is True and initialization.get("compute_device_type") == "cuda"
                 ...
                 and initialization.get("initial_x_t_equals_source") is True
                 and initialization.get("initialization_teacher_assisted") is False
                 and sha256(root / "initialization" / "source_only_initialization.pt")
...
def _plot(evidence: Mapping[str, Any], formal_run: Path, output: Path) -> None:
    ...
    figure.suptitle("Source-only initialization / teacher-MSE diagnostic | parent and new M0 differ")
    figure.tight_layout()
...
def _readme(evidence: Mapping[str, Any]) -> str:
    ...
    return f"""# R11_new：Source-only initialization 真实结果

训练锁定提交：`{evidence['training_commit']}`。固定 target 1，唯一干预是使用固定 source latent 初始化 xT；初始化不接收 teacher、query 或答案；完整四步 DreamLite 保持冻结，只优化 FP32 xT。
