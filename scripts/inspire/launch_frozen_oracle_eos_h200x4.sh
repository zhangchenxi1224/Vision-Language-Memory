...
exec "$runtime_python" -u scripts/experiments/run_frozen_oracle_eos_campaign.py \
  --config "$repo_dir/configs/experiments/frozen_oracle_eos.json" \
  --manifest "$repo_dir/configs/experiments/frozen_oracle_eos_manifest.json" \
  ...
  --output-root "$output_root" --expected-commit "$expected_commit" --max-hours 72
