...
**后续纠正与补充（01:19）**：原始样本速度 MSE 包含不可约条件方差，尤其 sigma=.5 时随机 teacher 的不可约 MSE 已达 0.409510，不能把它全部算成未拟合。新的闭式条件均值速度对照在同样四步、同样八个新噪声上为 8/8；U-Net 与该参考速度的差异集中在后段，沿自身轨迹最后一步 MSE 为 7.046。完整推导、结果与边界见 [设计审计](../unet-design-audit-20260909.md)。该对照读取整个成功库，不是已训练 U-Net 的成绩。
