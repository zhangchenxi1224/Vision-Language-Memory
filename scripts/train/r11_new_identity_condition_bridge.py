"""R11_new Target-01 identity-conditioning source-initialized latent bridge diagnostic.
...
CONFIG_PATH = (
    ...
    / "configs"
    / "experiments"
    / "r11_new_identity_conditioning_bridge_target01.json"
)
PROTOCOL = core.BRIDGE_PROTOCOL
IMPLEMENTATION_REVISION = "canonical-r11-latent-mse-identity-condition-source-init-target01-v1"
SUITE = "r11_new_target01_canonical_latent_bridge_identity_condition"
...
def _validate_args(args: argparse.Namespace) -> tuple[dict[str, Any], dict[str, Any]]:
    ...
    condition_binding.validate_probe_binding(config)
...
def _required_formal_artifacts(output_dir: Path) -> bool:
    required = {
        ...
        "condition/identity_condition_repeat.pt",
...
def _technical_gate(
    ...
    *,
    ...
    audit = {
        ...
        "condition_artifact_valid": condition_binding.verify_condition_record(manifest["condition_artifact"], config=_load_config()),
        ...
        "identity_condition_probe_binding_valid": condition_binding.validate_manifest_identity(manifest, config=_load_config()),
        "actual_conditioning_text_binding_valid": condition_binding.verify_condition_record(manifest["condition_artifact"], config=_load_config()),
    }
    return {
...
def _manifest(
    *,
    ...
    information_boundary = condition_binding.information_boundary(context["target"])
    ...
    return {
        ...
        "single_changed_factor": condition_binding.single_changed_factor(_load_config()),
        "identity_condition_probe_binding": dict(context["identity_condition_probe_binding"]),
        "fixed_contract": {
            ...
            "m0_definition": "the unoptimized endpoint after the complete four-step DreamLite path using the fixed identity conditioning and the unchanged source-only x_T initialization; new M0, never old-arm M0",
...
def _formal(
    *,
    ...
    conditioning_audit = core.bridge_conditioning_hypothesis_audit(
    ...
    conditioning_decision = core.bridge_conditioning_hypothesis_decision(
        distance_pass=distance_gate,
        reader_transfer_pass=reader_gate,
        audit=conditioning_audit,
    )
    return {
        ...
        "decision": decision,
        "secondary_solver_hypothesis_audit": conditioning_audit,
        "secondary_solver_hypothesis_decision": conditioning_decision,
        "phase2_allowed": False,
        "formal_success_gate": False,
        "initialization_teacher_assisted": False,
        "optimization_teacher_supervised": True,
...
def _run(
    ...
    *,
    ...
    processor, _pipe, reader, oracle, source_latents, context = condition_binding.load_runtime(args, _load_config())
