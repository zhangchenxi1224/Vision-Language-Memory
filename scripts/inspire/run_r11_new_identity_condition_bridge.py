"""Fail-closed Inspire controller for the identity-conditioning bridge diagnostic."""
...
TRAINER = ROOT / "scripts" / "train" / "r11_new_identity_condition_bridge.py"
CONFIG = (
    ROOT
    / "configs"
    / "experiments"
...
PREFLIGHT_REQUIRED_AUDIT_TRUE = (
    ...
    "identity_condition_probe_binding_valid", "actual_conditioning_text_binding_valid",
)
...
EXPECTED_HOST_PREFIX = "vlm-r11-identity-h200x4-20260907"
INSPIRE_SSD_ROOT = Path("/inspire/ssd")
MINIMUM_FREE_BYTES = 50 * 1024**3
LOCK_PATH = Path("/tmp/vision-memory-r11-new-identity-conditioning-bridge.lock")
...
def _validate_child(run: Path, *, mode: str, expected_commit: str) -> dict[str, Any]:
    ...
    common = {
        ...
        "identity_condition_binding_valid": condition_binding.validate_manifest_identity(manifest, config=trainer._load_config()),
    ...
    if mode == "technical-preflight":
        ...
        mode_checks = {
            ...
            "conditioning_audit_exact": summary.get("secondary_solver_hypothesis_audit")
            == conditioning_audit,
            "conditioning_audit_decision_exact": summary.get("secondary_solver_hypothesis_decision")
            == core.bridge_conditioning_hypothesis_decision(
                distance_pass=bool(
