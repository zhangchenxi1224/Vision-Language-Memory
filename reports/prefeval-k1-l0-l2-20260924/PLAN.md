...
在训练前缀首次产出、尚未读取其成绩时补充登记：将这 64 条单写模型真实链作为
保持训练前的 T1 MCQ 参照，各读 seed-0 的 0/5/10 PNG 和同主题错配图。
与保持后重新生成的相同 seed-0 端点配对；最终两噪声链主评测照常保留。
该参照来自训练侧前缀，不是未训练偏好或独立保持测试集，不与正式两链分母混合。
读取等待新偏好 MCQ 及全部前缀齐备，复用冻结评测实现；不阻止已登记的保持训练，
不据此改变本轮 2,048 步预算，不读取 OOD 来选择模型。
...
`education_resources:0043/0044/0050` 分别对应训练侧
`education_learning_styles:0011/0003/0004`。
保留官方划分和 180 条主指标；完成 730 条训练后，不把这三条声称为全新内容泛化，
可另列 3 条已暴露文本与 177 条无完全重复文本的补充结果。不得仅报告较好子集。
此检查不使用任何模型评测输出；文本无完全重复也不保证语义全新。
90 dev 与训练侧共享 16 个主题，因此该阶段检验同主题的新记录；180 official 属于另 4 个主题。
原始配对及输入文件哈希见 [文本重合记录](evidence/preference-text-overlap.json)。
...
当前首轮 GPU 仍执行原冻结 64 条实现。90 dev T1 已显示首次写入的新内容泛化失败，
据此并行准备 [730 条覆盖实验](SCALE730_PLAN.md)；首轮 RGB 保持继续完成。
新增代码不改变当前运行的首轮训练或检查点选择。
