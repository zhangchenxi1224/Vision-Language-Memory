...
09-14 15:55：03完整表达对照1721/1800已本地复核。相对03，4f修复39历史、新增10链错误、40链仍错；完整结果、逐格比较和全部归档见[03对照报告](03-observed-wording-baseline-review.md)。4f原功能1790/1800、已观察表达1750/1800、PNG3540/3600；当前仍未完整可用。以下历史记录按原时刻保留。

目标仍在进行。FM和采样偏差已成套修复并有数值证据，后续仍在依据实际失败检查条件编码等差异，目前还没有通过功能验收的 Writer。

## 09-14 06:30：raw完整对照仍56失败，历史表述增强实训已启动

raw四路1990条原始回答、360PNG及真实CLI现已全部本地重算通过；单次360/360、链480/480（16/16），历史904/960。相对同一bb权重原生推理，历史修复45条却新增36条错误，详见[完整raw功能报告](raw-condition-validation-review.md)。新b9f90e9固定预算历史表述覆盖对照已在四卡通过实际全U-Net梯度预检，正在完整baseline；新7b82309验证代码已加入逐draw表达/编码检查和独立终点PT审计，尚无新训练终点。

## 09-14 05:58：原生完整功能验证结束，历史改写仍失败64条

全部1990条raw、360张PNG和六写三十读CLI证据已下载、按独立远端SHA核对并本地重算。单次360/360、链480/480与16/16整链；历史lane0为416/480、lane1为480/480。相对旧bb，历史新增15条错误、修复16条，不能只看净增1条。CLI重放一致且该单链通过，完整suite仍失败，见[完整功能分析](native-condition-validation-review.md)。冻结bb的raw条件功能对照以新源码5ef8aa8在四张H200实际运行；35741bd首次启动因脚本根目录导入失败，未开始GPU验证，修复后使用独立目录重启，未修改任何模型或评分。

## 09-14 05:14：原生训练条件对齐的完整开发终点1510/1510

05:24补充：冻结bb旧终点、不更新权重，只改用raw训练条件推理的全矩阵对照也达到1510/1510。全部50条错误修复，1460条正确回答和全部负对照保持；新旧604PT经CPU实际检查，6040raw已本地重算，见[冻结条件对照完整证据](raw-condition-development-review.md)。这进一步支持条件不一致的影响；raw全功能suite仍须在原生suite之后分别验收。

同初始化、同4832更新和19328draw，只对齐训练条件编码的新03终点，音乐1350/1350、历史160/160，全部302张图五问法通过；上一版bb为1460/1510。完整终点证据和独立CPU最终PT检查均已下载，本地6040raw/19328draw及302条张量审计绑定重算通过，见[完整分析与证据](native-condition-development-review.md)。9e四路功能验证已实际占用四卡启动，随后还有真实CLI重放。开发通过仍不等于完整功能验收通过；下方为历史时刻记录。

## 09-14 03:42：原生训练条件对照在新四卡运行

03:58补充：新基线已完整通过与bb初始化的逐位比较，独立CPU再次重读604份两轮PT后通过；本地又复核全部3020条raw与首步四卡证据。基线1273/1510、250/302图五问法全对，音乐1230/1350、历史43/160，完全保持原对照起点。证据见[基线CPU摘要](03f8467-native-baseline-verified-summary.json)和[本地完整重算](03f8467-native-baseline-local-verification.json)。训练已实际推进到519/4832更新（422.5秒时的观察），后续完整优化与功能验证尚未结束。

bb34092逻辑抽样对照的四路验证全部结束并本地重算：单次280/360、连续链390/480与7/16整链、历史前缀895/960与166/192图全五问，见[完整配对结果](logical31-validation-review.md)。真实独立CLI6写30读重放一致，功能仍失败。

全部302格真实首步诊断已完成，10个失败格在native条件下平均速度MSE为0.16410525，raw条件下为0.000681887。速度、噪声、目标和真实Euler更新均已核验，见[本地诊断复核](17f35be-first-step-local-verification.json)。这不是新模型的功能准确率。

训练源码03f8467在新实例dl-official-exp-h200x4-20260914运行相同4832更新/19328draw对照，只将训练条件改成官方Base完整三分支编码的第三行；官方FM、28步/CFG1推理和测试矩阵保持。真实四卡/串行全模型梯度校验通过，目前重测全部基线，尚未观察新优化终点。固定9e27050验证源码已排队等待该终点，随后自动执行完整四路与CLI重放。见[条件训练计划](../official-native-condition-training-plan-20260914.md)和[续接记录](../official-alignment-continuation-20260913.md)。下方所有“正在执行”等句子是对应历史时刻的记录。

## 09-14 最新：采样对照完成，历史开发通过但音乐清除退步

同初始化、同4832更新、同151bank，仅改为31种逻辑条件均衡采样：历史37/160→160/160，音乐1340/1350→1300/1350，总计1460/1510，292/302图五问法全对。完整tensor/基线复核和本地6040raw/19328draw重算已通过；独立验证正在执行，见[对照分析](logical31-development-review.md)。原84端点的四路验证已全部远端完成、本地全raw/PNG重算并归档，单图360/360、链470/480（15/16整链）、历史170/960（1/192图五问法全对），见[完整独立验证](broader151-validation-review.md)。下方时间段为保留的历史记录，不表示当前仍在训练。

## 13:40 最新151条件端点已经完成，多题仍失败

固定4832更新/19328draw已结束。完整配对开发从1273/1510提升到1377/1510，但音乐条件1230/1350→1340/1350，历史16题43/160→37/160，没有一道历史题通过全部十次读取。50条原先正确的回答退步。远端检查604份PT和checkpoint，本地实际重算全部6040raw与19328draw，见[完整端点分析](broader151-endpoint-review.md)和[本地核验](c2ec407-broader-endpoint-local-verification.json)。数据中89.4%的采样来自同一音乐问题，是下一步要检验的分配问题，尚未据此证明唯一根因。四路独立验证正在实际运行；下方训练中表述为早期阶段记录。
